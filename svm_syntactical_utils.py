...
def process_text_for_syntax(raw: str, nlp_model):
    ...
    doc = nlp_model(raw)
    pos_seq = [t.pos_ for t in doc if not t.is_punct and not t.is_space]
...
def build_syntax_dataframe(metadata_path: str, transcripts_dir: str, output_path: str | None = None) -> pd.DataFrame:
    ...
    if nlp_model is None:
        print("\nERROR: spaCy model not available — cannot extract syntactic features.\n")
        return pd.DataFrame()

    print("Aktive spaCy-Pipes:", nlp_model.pipe_names)
