...
# --- Paths ---
metadata_path = "data/metadata_cl.xlsx"
transcripts_dir = "data/raw_data"
...
X_train_text = train_df['Cleaned Line']
y_train = train_df['Variety']
X_test_text = test_df['Cleaned Line']
...
for fold, (train_index, val_index) in enumerate(skf.split(df, df['stratify_label'])):
    fold_train = df.iloc[train_index].dropna(subset=['Cleaned Line']).copy()
    fold_val = df.iloc[val_index].dropna(subset=['Cleaned Line']).copy()

    X_fold_train_vect = vectorizer.fit_transform(fold_train['Cleaned Line'])
    y_fold_train = fold_train['Variety']

    X_fold_val_vect = vectorizer.transform(fold_val['Cleaned Line'])
...
# log Wahrscheinlichkeiten pro Klasse (shape: n_classes x n_features)
log_probs = model.feature_log_prob_

# Differenz der Wahrscheinlichkeiten zwischen Klassen
diff = log_probs[1] - log_probs[0]  # positiv = typisch La Paz, negativ = typisch Quito

# Feature-Namen aus CountVectorizer
features = np.array(vectorizer.get_feature_names_out())

# Top 10 typisch für La Paz
top10_lapaz = features[np.argsort(diff)[-10:][::-1]]

# Top 10 typisch für Quito
top10_quito = features[np.argsort(diff)[:10]]

print("Top 10 La Paz features:", top10_lapaz)
print("Top 10 Quito features:", top10_quito)
