...
INTERJECTION_SET = {
    "ay","bah","hum","huy","ah","eh","oh","uh","puaf","puaff","puf","puff",
    "uf","uff","aj","puaj","ejem","epa","hala","olé","ajá","ajajá","pche",
    "pchs","pst","buah","am","uhum","uhm","ehm","eeh","emm","aah"
}
...
def process_text(raw: str, nlp_model=None):
    """Clean text and optionally mask named locations."""
    text = raw
    if nlp_model:
        doc = nlp_model(text)
...
def build_cleaned_dataframe(metadata_path: str, transcripts_dir: str,
                            output_path: str | None = None,
                            use_spacy: bool = True) -> pd.DataFrame:
    """Create cleaned dataframe containing one row per interview turn."""
    meta = pd.read_excel(metadata_path).set_index("Interview ID")
    rows = []

    nlp_model = get_nlp() if use_spacy else None
    if use_spacy and nlp_model:
    ...
    for fn in sorted(os.listdir(transcripts_dir)):
        ...
        for turn, raw in enumerate(raw_lines, 1):
            text, tokens = process_text(raw, nlp_model=nlp_model)
